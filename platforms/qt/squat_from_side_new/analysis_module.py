...
def get_instructions() -> list:
    return [
        ...
        "Perform slow squats, bending your knees to at least a 60 degree angle while keeping your back naturally aligned."
    ]
    ...
    if _phase == "moving":
        ...
        if bend < 15.0:
            ...
            if _start_hip_y > 0.0 and _peak_hip_y > 0.0:
                completed = _peak_hip_y > _start_hip_y + 0.03
                _start_hip_y = 0.0
                _peak_hip_y = 0.0
                return completed
            return True
...
def generate_rep_feedback(rep_data: dict) -> list:
    ...
    leans = [l for l in leans if l > 0.5]
    if leans:
        peak_lean = max(leans)
        min_lean = min(leans)
        if peak_lean > 50.0:
            feedback.append("You're leaning too far forward with your upper body — keep your chest a little more upright.")
        if min_lean < 12.0:
            feedback.append("You're staying too upright — allow your torso to lean forward naturally as you squat.")

    # Knees over toes check (YOLO11 forward-ratio method)
    ...
    # Spine checks (baseline-relative, YOLO11 only)
    ...
    if baseline:
        ...
        if thoracic is not None and thoracic["delta"] > 0.15:
            feedback.append("Avoid rounding your upper back too much — try to keep your spine more neutral.")
        if lumbar is not None and lumbar["delta"] < -0.15:
            feedback.append("Avoid arching your lower back too much during the squat.")

    if len(feedback) == 1:
        feedback.append("Good squat — nice depth and steady form.")
...
def get_session_summary(session_data: dict) -> list:
    ...
    lines.append("You completed " + str(total_reps) + (" rep" if total_reps == 1 else " reps") + " this session.")
    ...
    if len(round_peaks) >= 2:
        if round_peaks[-1] > round_peaks[0] + 10:
            lines.append("Your squat depth improved across the session, from round 1 to round " + str(len(round_peaks)) + ".")
        elif round_peaks[0] > round_peaks[-1] + 10:
            lines.append("Your squat depth was better in the earlier rounds compared to round " + str(len(round_peaks)) + ".")
    ...
    if round_peaks and max(round_peaks) >= 60.0:
        lines.append("You reached a good depth of at least 60 degrees of knee bend during the session.")
    else:
        lines.append("Focus on increasing your squat depth in future sessions.")

    round_feedback = session_data.get("round_feedback", [])
    if round_feedback:
        def _has(text_list, keyword):
            for t in text_list:
                if keyword in t:
                    return True
            return False

        all_have_forward_knee = True
        any_round_feedback = False
        for rf in round_feedback:
            if rf:
                any_round_feedback = True
                if not _has(rf, "over your toes"):
                    all_have_forward_knee = False
        if any_round_feedback and all_have_forward_knee:
            lines.append("Letting your knees travel too far over your toes was a recurring issue — focus on pushing your hips back next time.")
